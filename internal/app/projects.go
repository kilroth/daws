package app

import (
	md "daws/internal/models"
	"daws/internal/utils"
)

func (a *App) DataLoad() (md.AppData, error) {
	var data md.AppData
	var err error

	projects, err := a.SM.GetAllProjects()
	if err != nil {
		return data, utils.Logger.Error("Failed to load projects: %v", err)
	}
	data.Projects = projects
	return data, nil
}

func (a *App) NewProject(project *md.Project) error {
	if project == nil {
		return utils.Logger.Error("Project cannot be nil")
	}
	if project.Name == "" {
		return utils.Logger.Error("Project name cannot be empty")
	}
	err := a.SM.SaveProject(project)
	if err != nil {
		return utils.Logger.Error("Failed to save project: %v", err)
	}
	return nil
}

func (a *App) GetProject(projectName string) (*md.Project, error) {
	if projectName == "" {
		return nil, utils.Logger.Error("Project name cannot be empty")
	}
	project, exists := a.Data.Projects[projectName]
	if !exists {
		return nil, utils.Logger.Error("Project not found: %s", projectName)
	}
	return &project, nil
}

func (a *App) UpdateProject(projectName string, project *md.Project) error {
	if project == nil {
		return utils.Logger.Error("Project cannot be nil")
	}
	if project.Name == "" {
		return utils.Logger.Error("Project name cannot be empty")
	}
	if _, exists := a.Data.Projects[projectName]; !exists {
		return utils.Logger.Error("Project not found: %s", projectName)
	}
	var err error
	// encrypt aws credentials before saving
	err = a.SM.SaveProject(project)
	if err != nil {
		return utils.Logger.Error("Failed to save project: %v", err)
	}
	return nil
}

func (a *App) DeleteProject(projectName string) error {
	if projectName == "" {
		return utils.Logger.Error("Project name cannot be empty")
	}
	if _, exists := a.Data.Projects[projectName]; !exists {
		return utils.Logger.Error("Project not found: %s", projectName)
	}
	err := a.SM.DeleteProject(projectName)
	if err != nil {
		return utils.Logger.Error("Failed to delete project: %v", err)
	}
	delete(a.Data.Projects, projectName)
	return nil
}
